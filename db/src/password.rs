...
pub fn verify_user_password(user: &User, other_password: Password) -> bool {
...
pub fn verify_password(pw_hash: &PasswordHash, other_password: Password) -> bool {
  let parsed_hash = argon2::password_hash::PasswordHash::new(&pw_hash.0).unwrap();
  let argon2 = Argon2::default();

  argon2.verify_password(other_password.0.as_bytes(), &parsed_hash).is_ok()
}
